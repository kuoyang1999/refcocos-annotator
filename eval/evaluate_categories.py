...
def calculate_accuracy_metrics(coco_data, results_data):
    ...
    for i in range(total_samples):
        ...
        hidden = categories.get('hidden', False)
        hidden_key = 'true' if hidden else 'false'
        metrics['hidden'][hidden_key]['total'] += 1
        if is_correct:
            metrics['hidden'][hidden_key]['correct'] += 1
...
def print_metrics(metrics, model_name=""):
    ...
    # 2. Calculate and print hidden accuracy
    hidden_data = metrics['hidden']['true']
    hidden_acc = (hidden_data['correct'] / hidden_data['total']) * 100 if hidden_data['total'] > 0 else 0
    print(f"\n2. Accuracy for Hidden = true: {hidden_acc:.1f}% ({hidden_data['correct']}/{hidden_data['total']})")
    ...
    print("\n4. Accuracy by Type:")
    for type_val in ['spatial', 'exclude', 'verb']:
...
def process_result_file(coco_data, results_path):
    ...
    export_metrics = {
        ...
        'hidden': {k: calculate_percentage(v) for k, v in metrics['hidden'].items()},
